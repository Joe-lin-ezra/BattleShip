...
public class GuishowFP{
	ClientFP client = null;
	...
	private ChessboardFP chessBoardz,chessBoarde;
	...
	int i,j,s;
	String state=null;
	public ArrayList<Location> location1 = new ArrayList<Location>(); // changing "Locate" to "Location"
	public ArrayList<Location> location2 = new ArrayList<Location>();
	...
	public final void initializeGui(){
		...
		chessBoarde = new ChessboardFP('e');
		chessBoardz = new ChessboardFP('y'); // make new chessboard
				...
				if(e.getActionCommand() == "Deploy"){
					...
					for (i = 0; i < 10; i++) {
						for (j = 0; j < 10; j++) {
							...
							{
								int t=chessBoardz.getplanetype(i,j);
								Location ddd = new Location(i,j);
								switch(t){
									case 1:
										location1.add(ddd);
										break;
								 ...
								 // changing "Locate" to "Location"
								//location.add(ddd);
								//System.out.println("(" + i + "," + j + ") is yellow");
							}
						}
					}
					if(location1.size()+location2.size()+location3.size()+location4.size()+location5.size() < 15){  //duplicate deploy
						area.append("your deploy is not ready!!!\n");
						//System.out.println("error");
					}else{
						client.setlocation(1,location1);
						client.setlocation(2,location2);
						client.setlocation(3,location3);
						client.setlocation(4,location4);
						client.setlocation(5,location5);
...
class ChessboardFP extends JPanel{// make chessboard
	private JPanel chessBoard;
	private JButton[][] chessBoardSquares = new JButton[10][10];
	private int[][] chessBoardplanetype = new int[10][10];
	int i,j;
	...
	ChessboardFP(char a){// constructor
		this.a = a;
	...
	public int getplanetype(int x,int y){
		return chessBoardplanetype[x][y];
	}
					...
					case 2:
						if(chessBoardSquares[x][y].getBackground() != Color.YELLOW && chessBoardSquares[x][y].getBackground() != Color.green)
						{
							if(chessBoardSquares[x+1][y].getBackground() != Color.YELLOW && chessBoardSquares[x+1][y].getBackground() != Color.green)
							{
								...
								if(mode==1){
									chessBoardplanetype[x][y] = 2;
									chessBoardplanetype[x+1][y] = 2;
									this.ship_longth=0;
								}
							}
						}
						...
						if(chessBoardSquares[x-1][y].getBackground() != Color.YELLOW && chessBoardSquares[x-1][y].getBackground() != Color.green)
						{
							if(chessBoardSquares[x][y].getBackground() != Color.YELLOW && chessBoardSquares[x][y].getBackground() != Color.green)
							{
								if(chessBoardSquares[x+1][y].getBackground() != Color.YELLOW && chessBoardSquares[x+1][y].getBackground() != Color.green)
								{
									...
									chessBoardSquares[x+1][y].setBackground(color);
									if(mode==1){
										chessBoardplanetype[x-1][y] = 3;
										chessBoardplanetype[x][y] = 3;
										chessBoardplanetype[x+1][y] = 3;
							...
							if(chessBoardSquares[x-1][y].getBackground() != Color.YELLOW && chessBoardSquares[x-1][y].getBackground() != Color.green)
							{
								if(chessBoardSquares[x][y].getBackground() != Color.YELLOW && chessBoardSquares[x][y].getBackground() != Color.green)
								{
									if(chessBoardSquares[x+1][y].getBackground() != Color.YELLOW && chessBoardSquares[x+1][y].getBackground() != Color.green)
									{
										...
										if(mode==1){
											chessBoardplanetype[x-2][y] = 4;
											chessBoardplanetype[x-1][y] = 4;
											chessBoardplanetype[x][y] = 4;
											chessBoardplanetype[x+1][y] = 4;
											this.ship_longth=0;
										}
									}
									...
									if(chessBoardSquares[x+1][y].getBackground() != Color.YELLOW && chessBoardSquares[x+1][y].getBackground() != Color.green)
									{
										if(chessBoardSquares[x+2][y].getBackground() != Color.YELLOW && chessBoardSquares[x+2][y].getBackground() != Color.green)
										{
											...
											if(mode==1){
												...
												chessBoardplanetype[x-2][y] = 5;
												chessBoardplanetype[x-1][y] = 5;
												chessBoardplanetype[x][y] = 5;
												chessBoardplanetype[x+1][y] = 5;
												chessBoardplanetype[x+2][y] = 5;
												this.ship_longth=0;
												//System.out.println("ship_length is " + this.ship_longth);
							...
							if(chessBoardSquares[x][y-1].getBackground() != Color.YELLOW && chessBoardSquares[x][y-1].getBackground() != Color.green)
							{
								if(chessBoardSquares[x][y].getBackground() != Color.YELLOW && chessBoardSquares[x][y].getBackground() != Color.green)
								{
									if(chessBoardSquares[x][y+1].getBackground() != Color.YELLOW && chessBoardSquares[x][y+1].getBackground() != Color.green)
									{
										...
										chessBoardSquares[x][y+1].setBackground(color);
										if(mode==1){
											chessBoardplanetype[x][y-2] = 4;
											chessBoardplanetype[x][y-1] = 4;
											chessBoardplanetype[x][y] = 4;
											chessBoardplanetype[x][y+1] = 4;
											this.ship_longth=0;
										}
									...
									if(chessBoardSquares[x][y+1].getBackground() != Color.YELLOW && chessBoardSquares[x][y+1].getBackground() != Color.green)
									{
										if(chessBoardSquares[x][y+2].getBackground() != Color.YELLOW && chessBoardSquares[x][y+2].getBackground() != Color.green)
										{
											...
											if(mode==1){
												//System.out.println("deploy!!!");
												//System.out.println("ship_length is " + this.ship_longth);
												chessBoardplanetype[x][y-2] = 5;
												chessBoardplanetype[x][y-1] = 5;
												chessBoardplanetype[x][y] = 5;
												chessBoardplanetype[x][y+1] = 5;
												chessBoardplanetype[x][y+2] = 5;
												this.ship_longth=0;
	...
	public void mkchessboard(){
		...
		for(i = 0; i < chessBoardSquares.length; i++){ 
			for(j = 0; j < chessBoardSquares[i].length ; j++){
				...
				int y=i; // button ID x
				int x=j; // button ID y
				//int plane_type=0;//fighterplane type; no payload is 0(default)
